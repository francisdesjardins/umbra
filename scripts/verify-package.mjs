...
try {
  ...
  writeFileSync(
    ...
    [
      "import { useModal, useMessageModal, useSlideModal, useModalActions } from 'umbra/react';",
      "import { defineAction, ModalOutlet, useStore, dialogManager } from 'umbra/react';",
      'export const used = [useModal, useMessageModal, useSlideModal, useModalActions,',
      '  defineAction, ModalOutlet, useStore, dialogManager];',
    ].join('\n')
  );
  ...
  writeFileSync(
    ...
    [
      ...
      "import { useModal, useModalActions, defineAction } from 'umbra/react';",
      ...
      'export const modalType = info.exists ? info.modalType : undefined;',
      '// @ts-expect-error registration-time facts need narrowing on `exists`',
      'export const unguarded = info.modalType;',
      '',
      ...
      "typed.close('ok', 42);",
      '',
      '// The payload is declared once, on the action, and inferred from there — no type',
      '// argument on `useModal`. This only holds if `ActionsBinding` survives into the',
      '// published declarations with its payload intact, symbol key and all.',
      'const actions = useModalActions({',
      '  save: defineAction<{ id: string }>(),',
      '  cancel: defineAction(),',
      '});',
      'const inferred = useModal({',
      "  id: 'inferred',",
      '  actions,',
      '  render: () => null,',
      '  onClose: (result) => {',
      ...
      '  },',
      '});',
      "inferred.handle.close('save', { id: 'a' });",
      '// @ts-expect-error the inferred payload is the action set’s, not `unknown`',
      "inferred.handle.close('save', 42);",
    ].join('\n')
  );
