...
| `onDismissRequest?`       | `() => boolean \| void`                                          | Report the dismiss key instead of acting on it, for a surface whose open state is a prop. Replaces the **last** step only — which key, whether an action claimed it, whether a popup answers it first, whether `prepare` or a running action forbids it, and which dialog is in front all stay the library's. Return `false` to decline the press, which only the non-modal window listener acts on: it captures, so a press it takes is one the page never sees. Reaches `useMessageModal` and `useSlideModal` unchanged.                                                                                                                                                               |
...
**The other end of that window is refused, not answered.** Called while the modal is `'closing'`,
`openAndWait` resolves `[Error, null]` — `Modal "x" is closing; no reopen is queued`. No reopen is
queued (see `open`), so the exit already running belongs to whoever asked for it; handing its reason
back would report a decision this caller never caused, for a dialog it never saw. All three awaiting
doors share the rule, since all three register their resolver on the same store: the hook's
`openAndWait`, `dialogManager.openAndWait`, and `requestOpenAndWait` — whose accepted outcome then
carries the error branch on `closed`.
