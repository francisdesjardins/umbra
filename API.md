...
| `FocusOptions.focusVisible`                        | enhancing    | 145         | 104     | 18.4   | core/focus-policy.ts — `SHOW_THE_RING`, on every focus move the library makes of its own. **Below:** Input modality decides instead, which is what an ordinary page does. A dialog opened **from the keyboard** still rings: `:focus-visible` propagates to a scripted focus from an element that already matched it, so that path is unaffected on every engine. A dialog opened **by pointer** does not — the move is made, and it is silent. Measured both ways with the option neutralised. ([source](https://caniuse.com/mdn-api_htmlelement_focus_options_focusvisible_parameter), read 2026-08-31) |
