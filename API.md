...
- **`umbra/react`** — the React binding (all hooks, `DialogOutlet`), **plus a wholesale re-export
...
2. **Template hooks** — `useMessageDialog`, `useSlideDialog` provide headless modal logic (animation, positioning). No UI wrapper — users provide their own components in the `render` callback.
...
5. **`DialogOutlet`** — Optional portal manager. Wrap a subtree to auto-render modals without placing `{modal.Modal}` in JSX.
...
```tsx
const modal = useDialog<void, 'save' | 'dismiss'>({
...
| `Modal`         | `ReactNode`                   | Place in JSX as `{Modal}`; `null` when closed, and inside a `DialogOutlet`                 |
...
const modal = useMessageDialog({ id: 'fast', animation: NO_ANIMATION, render: () => ... });
```
...
Wrap a subtree so that every `useDialog` inside auto-registers its dialog — no `{modal.Modal}` in JSX required. `modal.Modal` becomes `null` inside an outlet; destructuring still works, it just renders nothing. Outlets are nestable: the nearest ancestor outlet wins.

```typescript
import { DialogOutlet, useDialog } from 'umbra/react';
...
| ---------- | ----------- | ----------------------------------------------------------------- |
| `children` | `ReactNode` | Subtree that can use `useDialog` without placing `{Modal}` in JSX |
...
all.getOpen('modal'); // DialogInfo[] — only showModal() dialogs
...
| -------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------- |
| `template`                 | `string`  | The label its creator gave it — `'modal'` by default, `'slide'` from `useSlideDialog`, anything your own template reports |
...
modals, so they are typed `RegisteredDialogInfo` and need no narrowing at all.
...
| `getOpen(filter?)`     | `DialogInfo[]`            | Open modals in stack order; filter `'modal'`/`'non-modal'`  |
...
| `getClosed()`          | `DialogInfo[]`            | All registered but closed modals                            |
...
| `nonModal`               | TYPE    | **Excludes** dismissOnBackdropClick when true, dismissOnClickOutside when false. **The one pair the checker holds.** `DialogVariant` is a discriminated union, so the wrong dismissal option for the variant is a compile error rather than an option that is silently read by nobody.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
`useDialog` defaults to `'modal'`; the built-in templates name themselves — `useMessageDialog`
...
| Property      | Type                      | Description                                                               |
| ------------- | ------------------------- | ------------------------------------------------------------------------- |
| `openDialogs` | `readonly DialogInfo[]`   | Open modals (modal and nonModal), in stack order — index = stack position |
...
Reactive hook for querying a single modal's state. Returns `DialogInfo` that updates whenever any modal opens, closes, registers, or unregisters. Uses `useSyncExternalStore` for tear-free reads.
...
| `modal`               | `useDialog` core — open/close requests                    |
