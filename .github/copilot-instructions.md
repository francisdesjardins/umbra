...
- **Ids may be declared**: a project augments `ModalRegistry` to give an id its `data` and `reason`; declaring is optional and per modal, so an undeclared id must keep working everywhere.
