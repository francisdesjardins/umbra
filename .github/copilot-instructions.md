...
- **Ids may be declared**: a project augments `ModalRegistry` to give an id its `closesWith` (the reasons, or a payload per reason) and its `payload`; declaring is optional and per modal, so an undeclared id must keep working everywhere. A payload declared for a reason is required when closing with it.
