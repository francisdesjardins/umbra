...
test.describe('resolveAnimation', () => {
  ...
  test('agrees with the transition the style builder emits', () => {
    ...
    const transition = String(getDialogAnimationStyles('closing', anim).transition);
...
test.describe('getDialogAnimationStyles', () => {
  test('includes base dialog reset styles', () => {
    const styles = getDialogAnimationStyles('closing', baseAnimation);
  ...
  test('applies entrance CSS while open', () => {
    const styles = getDialogAnimationStyles('open', baseAnimation);
  ...
  test('falls back to duration when exitDuration is omitted', () => {
    ...
    const styles = getDialogAnimationStyles('closing', anim);
    expect(styles.transition).toContain('200ms');
  });
  ...
  test('defaults duration to 200ms when not set', () => {
    const anim: ModalAnimation = { entrance: { opacity: 1 }, exit: { opacity: 0 } };
    const styles = getDialogAnimationStyles('open', anim);
    expect(styles.transition).toContain('200ms');
  });

  test('merges custom styles', () => {
    const styles = getDialogAnimationStyles('closing', baseAnimation, { color: 'red' });
  ...
  test('animation styles override custom styles for the same property', () => {
    ...
    const styles = getDialogAnimationStyles('closing', baseAnimation, { opacity: 0.5 });
    expect(styles.opacity).toBe(0);
  });

  test('applies the placement it is handed, and none of its own', () => {
    const portaled = getDialogAnimationStyles(
      'closing',
      baseAnimation,
      undefined,
      dialogPlacement({ nonModal: true, portal: true })
    );
    expect(portaled.position).toBe('fixed');

    const contained = getDialogAnimationStyles(
      'closing',
      baseAnimation,
      undefined,
      dialogPlacement({ nonModal: true })
    );
    expect(contained.position).toBe('absolute');

    // A modal dialog is placed by the top layer — nothing here may position it.
    expect(getDialogAnimationStyles('closing', baseAnimation).position).toBeUndefined();
  });
  ...
  test('a closed dialog is out of layout, whatever else it was given', () => {
    ...
    const closed = getDialogAnimationStyles(
      'closed',
      baseAnimation,
      { width: 200 },
      dialogPlacement({ nonModal: true })
    ...
    expect(getDialogAnimationStyles('open', baseAnimation).display).toBe('flex');
