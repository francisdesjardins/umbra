...
export const PLATFORM_ROWS: readonly PlatformRow[] = [
  ...
  },
  {
    ...
    caveat:
      '**The floor’s scan is not scoped to the dialog’s own subtree** the way every other lookup in `focus-policy.ts` is: `reclaimFocus` finds the `focusOnOpen` claim with `queryOwn` and then falls through to a plain `querySelectorAll`. It is not reachable *through the floor* — opening a nested dialog means clicking a button inside this one, which records `lastFocusInside`, and a recorded preference is what stops the floor running at all — so the arrangement to look at is the other caller, `attachFocusContainment`, whose Tab recovery shares the scan and can meet a nested **non-modal** panel (a nested modal one puts the top layer over this dialog’s dead space, so the press that starts it cannot be made). Argued from the code, not measured, and left open on that basis.',
  },
  {
